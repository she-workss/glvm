...
vec3 ComputePointLight(PointLight light, vec3 normal, vec3 fragmentPosition,
    vec3 viewDirection);

vec3 ComputeSpotLight(SpotLight light, vec3 normal, vec3 fragmentPosition,
    vec3 viewDirection);

float ComputeDirectionalShadow(DirectionalLight light,
    vec4 fragmentPositionDirectionalLightSpace, sampler2D flatShadowMap);

float ComputePointShadow(PointLight light, vec3 fragmentPosition,
    samplerCube cubeShadowMap);

float ComputeSpotShadow(SpotLight light, vec4 fragmentPositionSpotLightSpace,
...
void main() {
    ...
    if (sampledDirectionalShadowOrdinalNumbersArraySize > 0) {
        directionalLightIndexAccumulator =
            sampledShadowOrdinalNumbers[directionalLightIndicesCounter];
    }
    float shadow = 0.0;
    for (int f = 0; f < directionalLightsArraySize; ++f) {
        if (f == directionalLightIndexAccumulator &&
            directionalLightIndicesCounter !=
            sampledDirectionalShadowOrdinalNumbersArraySize) {
            shadow = ComputeDirectionalShadow(directionalLights[f],
                fs_in.fragmentPositionDirectionalLightSpace[f],
                directionalLightFlatShadowMapArray[f]);
            ++directionalLightIndicesCounter;
            directionalLightIndexAccumulator =
            sampledShadowOrdinalNumbers[directionalLightIndicesCounter];
        }
        vec3 light = ComputeDirectionalLight(directionalLights[f], normal,
            viewDirection);
    ...
    if (sampledPointShadowOrdinalNumbersArraySize > 0) {
        pointLightIndexAccumulator =
            pointLightCubeShadowMapComponentIndices[pointLightIndicesCounter];
    }
    shadow = 0.0;
    for (int i = 0; i < pointLightsArraySize; ++i) {
        if (i == pointLightIndexAccumulator &&
            pointLightIndicesCounter !=
            sampledPointShadowOrdinalNumbersArraySize) {
            shadow =
                ComputePointShadow(pointLights[i], fs_in.fragmentPosition,
        ...
        vec3 light = ComputePointLight(pointLights[i], normal,
            fs_in.fragmentPosition, viewDirection);
        result += (1.0 - shadow) * light;
        if (shadow > 0.0) {
            shadow = 0.0;
        }
    ...
    if (sampledSpotShadowOrdinalNumbersArraySize > 0) {
        spotLightIndexAccumulator =
            spotLightFlatShadowMapComponentIndices[spotLightIndicesCounter];
    }
    shadow = 0.0;
    for (int j = 0; j < spotLightArraySize; ++j) {
        vec3 lightDirection =
            normalize(spotLights[j].position - fs_in.fragmentPosition);
        float theta = dot(lightDirection, normalize(-spotLights[j].direction));
        if (j == spotLightIndexAccumulator &&
            spotLightIndicesCounter !=
            sampledSpotShadowOrdinalNumbersArraySize) { // Delete this
            shadow =
                ComputeSpotShadow(spotLights[j],
                    fs_in.fragmentPositionSpotLightSpace[j],
                    spotLightFlatShadowMapArray[spotLightIndicesCounter]);
            ++spotLightIndicesCounter;
            spotLightIndexAccumulator =
                spotLightFlatShadowMapComponentIndices[spotLightIndicesCounter];
        }
        vec3 light = ComputeSpotLight(spotLights[j], normal,
            fs_in.fragmentPosition, viewDirection);
        result += (1.0 - shadow) * light;
        if (shadow > 0.0) {
            shadow = 0.0;
...
vec3 ComputeDirectionalLight(DirectionalLight light, vec3 normal,
    ...
    float specularComponent =
        pow(max(dot(viewDirection, reflectDirection), 0.0f),
            material.shininess);
    // Combine results
    vec3 ambient = light.ambient * material.ambient;
    vec3 diffuse = light.diffuse * difference *
        vec3(texture(material.diffuse, fs_in.textureCoords));
    vec3 specular = light.specular * specularComponent *
        vec3(texture(material.specular, fs_in.textureCoords));
...
vec3 ComputePointLight(PointLight light, vec3 normal, vec3 fragmentPosition,
    ...
    // Specular shading
    vec3 reflectDirection = reflect(-lightDirection, normal);
    float specularComponent = pow(max(dot(viewDirection, reflectDirection),
...
float ComputeDirectionalShadow(DirectionalLight light,
    ...
    // fragmentPositionLight as coordinates)
    ...
    for (int x = -1; x <= 1; ++x) {
        for (int y = -1; y <= 1; ++y) {
            float pcfDepth = texture(flatShadowMap, projectiveCoordinates.xy +
...
float ComputePointShadow(PointLight light, vec3 fragmentPosition,
    ...
    for (int i = 0; i < samples; ++i) {
        float closestDepth = texture(cubeShadowMap, fragmentToLight +
...
float ComputeSpotShadow(SpotLight light, vec4 fragmentPositionSpotLightSpace,
    sampler2D flatShadowMap) {
    // Perform perspective devide
    vec3 projectiveCoordinates = fragmentPositionSpotLightSpace.xyz /
        fragmentPositionSpotLightSpace.w;
    // Transform to [0.1] range
    projectiveCoordinates = projectiveCoordinates * 0.5 + 0.5;
    // Get closest depth value from light's perspective (using [0,1] range
    // fragmentPositionLight as coordinates)
