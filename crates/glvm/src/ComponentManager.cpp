...
ComponentManager::~ComponentManager() {
    for (int i = 0, iSize_Main = worldComponentsContainer.GetSize();
         i < iSize_Main;
         ++i) {
    ...
    for (int j = 0,
             iSize_Ordered = worldSparseEntitiesMapToComponents.GetSize();
         j < iSize_Ordered;
         ++j) {
        delete worldSparseEntitiesMapToComponents[j];
        worldSparseEntitiesMapToComponents[j] = nullptr;
    }
    for (int j = 0, iSize_Ordered = worldDenseComponentsMapToEntities.GetSize();
         j < iSize_Ordered;
         ++j) {
...
bool ComponentManager::checkAvailability(
    ...
    return entity < sparse.GetSize() && sparse[entity] < dense.GetSize()
