...
void ImGuiOverlay::recordCommandBuffer(
    ...
    if (lineVertexCount_ > 0) {
        ...
        mat4 viewProj = renderer_.viewMatrix * renderer_.projectionMatrix;
        vkCmdPushConstants(
            commandBuffer,
            lineLayout_,
...
void ImGuiOverlay::buildPanel() {
    ...
    ImGui::Text("Actors: %u", renderer_.actors.GetSize());
    ImGui::Text(
...
void ImGuiOverlay::buildDebugVertices() {
    ...
    if (showActorBounds) {
        ...
        mins.reserve(renderer_.actors.GetSize());
        maxs.reserve(renderer_.actors.GetSize());
        for (size_t i = 0; i < renderer_.actors.GetSize(); ++i) {
            RenderActor actor = renderer_.actors[i];
            if (actor.meshID >= allMeshMaxAbsoluteValues.GetSize()) {
            ...
            vec3 localCorners[8] = {
                center + vec3(-half[0], -half[1], -half[2]),
                center + vec3(half[0], -half[1], -half[2]),
                center + vec3(half[0], half[1], -half[2]),
                center + vec3(-half[0], half[1], -half[2]),
                center + vec3(-half[0], -half[1], half[2]),
        ...
        std::vector<bool> collides(renderer_.actors.GetSize(), false);
        for (size_t i = 0; i < renderer_.actors.GetSize(); ++i) {
            for (size_t j = i + 1; j < renderer_.actors.GetSize(); ++j) {
    ...
    if (showLightFrustums) {
        ...
        for (uint32_t i = 0; i < renderer_.directionalLightNumber; ++i) {
            ...
            for (int c = 0; c < 8; ++c) {
                ...
                corners[c] = fromVec4(vec4(u, v, s, 1.0f) * inverseLight);
            }
        ...
        for (u32 z = 0; z < grid.depth; ++z) {
            for (u32 y = 0; y < grid.height; ++y) {
                for (u32 x = 0; x < grid.width; ++x) {
                    ...
                    const size_t count = chunk.entities.GetSize();
                    if (count == 0) {
                        continue;
                    }
                    // Only occupied cells, colored by entity count.
                    const vec3 color = count == 1
                        ? vec3(0.0f, 1.0f, 0.0f)
                        : count <= 3 ? vec3(1.0f, 1.0f, 0.0f)
                                     : vec3(1.0f, 0.0f, 0.0f);
                    const vec3 center = chunk.position
                        + vec3(halfChunk, halfChunk, halfChunk);
                    pushLine(vertices, center - vec3(cross, 0, 0), center + vec3(cross, 0, 0), color);
                    pushLine(vertices, center - vec3(0, cross, 0), center + vec3(0, cross, 0), color);
                    pushLine(vertices, center - vec3(0, 0, cross), center + vec3(0, 0, cross), color);
                }
            }
        }
    }
...
void ImGuiOverlay::createLinePipeline() {
    ...
    VkShaderModule vert = createShaderModule(
        "../../../assets/shaders/vk_shaders/debug/debug_vert.spv"
    );
    VkShaderModule frag = createShaderModule(
        "../../../assets/shaders/vk_shaders/debug/debug_frag.spv"
    );
