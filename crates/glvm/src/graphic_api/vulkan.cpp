...
#include "glvm/components/interface_component.hpp"
...
void CVulkanRenderer::createDescriptorSetLayout() {
    for (int descriptorSetCounter = 0;
        ...
        for (uint32_t j = 0; j < descriptorSet.actualLinkedDescriptorBindingsNumber;
             ++j) {
...
void CVulkanRenderer::createBuffer(
    ...
    int32_t result = vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory);
...
void CVulkanRenderer::fontRecordCommandBuffer(
    ...
    for (unsigned int i = 0; i < fonts.size(); ++i) {
        ...
        Vector<float, 3> playerTragetDirection = font.position - player.position;
        ...
        for (unsigned int j = 0; j < font.font_string.size(); ++j) {
            ...
            Vector<float, 4> pos =
                Vector<float, 4>(font.position[0], font.position[1], font.position[2], 1.0f);

            Vector<float, 4> clipSpacePosition = pos * viewMatrix * projectionMatrix;
...
void CVulkanRenderer::updateViewPositionUniformBuffer(
    ...
    print = false;
    lightDataUBO.tilesetTilesCount = Vector<float, 2>(TILESET_ROW, TILESET_COLUMN);
