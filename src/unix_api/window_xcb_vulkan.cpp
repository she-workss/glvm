...
WindowXCBVulkan::WindowXCBVulkan() {
    ...
    event_flags[1] = XCB_EVENT_MASK_BUTTON_PRESS |
    ...
    /// Make sure commands are sent befour we pause so that the window gets
    /// shown
...
xcb_window_t WindowXCBVulkan::GetWindow() {
    return window;
}
            ...
            case XCB_KEY_RELEASE: {
                ...
                if (next_generic_event != NULL) {
                    ...
                    xcb_keysym_t press_keysym = xcb_key_press_lookup_keysym(
                            key_symbols, key_press_event, 0);
                    xcb_keysym_t release_keysym = xcb_key_press_lookup_keysym(
                            key_symbols, key_release_event, 0);
...
void WindowXCBVulkan::CursorLock([[maybe_unused]] int _x_position,
