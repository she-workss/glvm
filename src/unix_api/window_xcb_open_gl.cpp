...
int WindowXCBOpengl::main_loop(Display *display, xcb_connection_t *connection,
                               [[maybe_unused]] xcb_window_t window,
...
void WindowXCBOpengl::setup_and_run(Display *display,
    ...
    xcb_create_window(connection, XCB_COPY_FROM_PARENT, window, screen->root, 0,
                      0, 1920, 1080, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, visualID,
                      valuemask, valuelist);
...
void WindowXCBOpengl::HideCursor() {
    ...
    xcb_create_gc(connection, graphical_context, window,
                  XCB_GC_FOREGROUND | XCB_GC_BACKGROUND, values_list);
    ...
    xcb_put_image(connection, XCB_IMAGE_FORMAT_XY_PIXMAP, foreground_pixmap_id,
                  graphical_context, 0, 0, 100, 100, 0, 8, 32, pix_map_data);

    xcb_cursor_t cursor = xcb_generate_id(connection);
    xcb_create_cursor(connection, cursor, foreground_pixmap_id,
                      foreground_pixmap_id, 0, 0, 0, 0, 0, 0, 8, 8);
...
xcb_connection_t *WindowXCBOpengl::GetConnection() {
    return connection;
}

xcb_window_t WindowXCBOpengl::GetWindow() {
    return window;
}

Display *WindowXCBOpengl::GetDisplay() {
    return display;
}

void WindowXCBOpengl::Disconnect() {
...
xcb_keysym_t WindowXCBOpengl::convertKeyCodeToSym(
        xcb_key_release_event_t *key_release_event) {
...
void WindowXCBOpengl::CursorLock([[maybe_unused]] int _x_position,
