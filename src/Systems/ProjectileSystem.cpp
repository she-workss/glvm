...
void CProjectileSystem::Update() {
    ...
    cm::beholder *view_Component =
            pComponent_Manager->GetComponent<cm::beholder>(iEntity_refView);
    ...
    for (unsigned int i = 0; i < u_iVector_Move_Size; ++i) {
        for (int n = 0; n < 6; ++n) {
            ...
            if (inputStack.SearchElement(core::EEvents::eMOUSE_LEFT_BUTTON) ==
                core::EEvents::eMOUSE_LEFT_BUTTON) {
                if (projectileCooldown <= 0) {
                    CalculateProjectile(pComponent_Manager, iEntity_refMove,
                                        *view_Component);
    ...
    for (unsigned int x = 0; x < linkedEntities.GetSize(); ++x) {
        ...
        rTransformProjectile->tPosition +=
                rTransformProjectile->tForward * cameraSpeed;
    ...
    for (unsigned int i = 0; i < linkedEntities.GetSize(); ++i) {
        ...
        if (pComponent_Manager
                    ->GetComponent<cm::collider>(uiEntity_refProjectile)
                    ->bWall_Collision_ ||
            pComponent_Manager
                    ->GetComponent<cm::collider>(uiEntity_refProjectile)
                    ->bGround_Collision_) {
...
void CProjectileSystem::CalculateProjectile(
        ecs::ComponentManager *componentManager, unsigned int entityRefMove,
        components::beholder &beholder) {
    namespace cm = GLVM::ecs::components;

    unsigned int uiEntity_Projectile =
            ecs::EntityManager::GetInstance()->CreateEntity();
    ecs::ComponentManager::GetInstance()
            ->CreateComponent<cm::mesh, cm::collider, cm::transform,
                              cm::material, cm::projectile, cm::pointLight>(
                    uiEntity_Projectile);
    ...
    componentManager->GetComponent<cm::mesh>(uiEntity_Projectile)->handle =
            meshHandle;
    ecs::TextureHandle textureHandle {};
    if (textureHandlers.GetSize() > 0) {
        textureHandle = textureHandlers[0];
    }
    cm::material *rTextureProjectile =
            componentManager->GetComponent<cm::material>(uiEntity_Projectile);
    *rTextureProjectile = {.diffuseTextureID_ = textureHandle,
                           .specularTextureID_ = textureHandle,
                           .ambient = {0.05f, 0.05f, 0.05f},
                           .shininess = 128.0f * 0.078125f};
    cm::transform *rTransformProjectile =
            componentManager->GetComponent<cm::transform>(uiEntity_Projectile);
    ...
    *(componentManager->GetComponent<cm::pointLight>(uiEntity_Projectile)) = {
            .position = rTransformProjectile->tPosition,
            .ambient = {0.1f, 0.1f, 0.1f},
            .diffuse = {0.5f, 0.5f, 0.5f},
            .specular = {1.1f, 1.2f, 1.3f},
            .constant = 1.4f,
            .linear = 0.1f,
            .quadratic = 0.128f};
}
...
Vector<float, 3>
CProjectileSystem::GetDirectionVector(components::beholder &beholder) {
