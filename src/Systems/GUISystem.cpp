...
void CGUISystem::Update() {
    ...
    //		tProjection_Matrix = Perspective(Radians(90.0f), (float)1920 /
    //(float)1080, 1.0f, 100.0f);
    ...
    //         unsigned int uiTransformt_Loc2 =
    //         pGLGet_Uniform_Location(_Shader_Program->iID, "modelMatrix");
    // 		pGLUniform_Matrix4fv(uiTransformt_Loc2, NUMBER_OF_MATRICES,
    // GL_FALSE, &tModel_Matrix[0][0]);

    // 		tProjection_Matrix = Perspective(Radians(90.0f), (float)1920 /
    // (float)1080, 1.0f, 100.0f);
    // 		pGLUniform_Matrix4fv(transformLocationProjection,
    // NUMBER_OF_MATRICES, GL_FALSE, &tProjection_Matrix[0][0]);
    ...
    //         pGLBuffer_Data(GL_ARRAY_BUFFER, sizeof(ray), ray,
    //         GL_DYNAMIC_DRAW);

    //         pGLVertex_Attrib_Pointer(0, VERTEX_SIZE, GL_FLOAT, GL_FALSE, 3 *
    //         sizeof(float), (void*)VERTEX_OFFSET);
    //         pGLEnable_Vertex_Attrib_Array(LAYOUT_0);
    ...
    //         necessary. pGLBind_Vertex_Array(0);
}
...
void CGUISystem::RaycastringDebug() {
    ...
    // 		unsigned int location = pGLGet_Uniform_Location(debugLines->iID,
    // "modelMatrix"); 		pGLUniform_Matrix4fv(location, NUMBER_OF_MATRICES,
    // GL_FALSE, &planeModelMatrix[0][0]);
    // 		// ComputeProjectionMatrix(debugLines);
    // 		// ComputeViewMatrix(debugLines, *playerTransformComponent,
    // *playerViewComponent); 		pGLGen_Vertex_Arrays(1, &vaoPlane);
    // 		pGLGen_Buffers(1, &vboPlane);
    // 		pGLBind_Vertex_Array(vaoPlane);
    // 		pGLBind_Buffer(GL_ARRAY_BUFFER, vboPlane);
    // 		pGLBuffer_Data(GL_ARRAY_BUFFER, sizeof(plane), plane,
    // GL_STATIC_DRAW);

    // 		pGLVertex_Attrib_Pointer(LAYOUT_0, VERTEX_SIZE, GL_FLOAT, GL_FALSE,
    // 6 * sizeof(float), (void*)VERTEX_OFFSET);
    // 		pGLEnable_Vertex_Attrib_Array(LAYOUT_0);
    // 		pGLVertex_Attrib_Pointer(LAYOUT_1, 3, GL_FLOAT, GL_FALSE, 6 *
    ...
    // 		pGLBind_Buffer(GL_ARRAY_BUFFER, vboLines);
    // 		pGLBuffer_Data(GL_ARRAY_BUFFER, sizeof(lines), lines,
    // GL_DYNAMIC_DRAW);
