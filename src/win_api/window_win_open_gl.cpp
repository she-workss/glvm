...
window_win_open_gl::window_win_open_gl() {
    ...
    int iClassic_Pixel_Format =
            ChoosePixelFormat(pClassic_DC_, &classic_Format_Descriptor);
    ...
    (HDC hdc, const int *piAttribIList, const FLOAT *pfAttribFList,
     UINT nMaxFormats, int *piFormats, UINT *nNumFormats) =
            (BOOL(WINAPI *)(HDC hdc, const int *piAttribIList,
                            const FLOAT *pfAttribFList, UINT nMaxFormats,
                            int *piFormats, UINT *nNumFormats))
                    wglGetProcAddress((LPCSTR) "wglChoosePixelFormatARB");

    HGLRC(WINAPI * WGLCreateContextAtribbARB)
    (HDC hDC, HGLRC hShareContext, const int *attribList) = (HGLRC(WINAPI *)(
            HDC hDC, HGLRC hShareContext, const int *attribList))
            wglGetProcAddress((LPCSTR) "wglCreateContextAttribsARB");
    ...
    pModern_Window_ = CreateWindowEx(0, aClass_Name, "Sample Window Class",
...
void window_win_open_gl::SwapBuffers() {
    ::SwapBuffers(pModern_DC_);
}

void window_win_open_gl::ClearDisplay() {
...
void window_win_open_gl::Close() {
    DestroyWindow(pModern_Window_);
    PostQuitMessage(0);
}

HWND window_win_open_gl::GetClassicWindowHWND() {
    return pClassic_Window_;
}
HWND window_win_open_gl::GetModernWindowHWND() {
    return pModern_Window_;
}

void window_win_open_gl::CursorLock(int _x_position, int _y_position,
